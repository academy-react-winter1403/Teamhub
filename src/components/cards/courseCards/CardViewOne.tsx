...
const CardViewOne = () => {
  return (
    <div className="bg-white shadow-md rounded-2xl mb-5 p-4 w-65 h-80">
        <img 
        ...
        />
        <div className="mt-3">
          <h3 className="title font-medium text-right text-md mr-3">{"دوره جامع .net core صفر تا صد"}</h3>

          <div className="flex gap-4 items-center text-gray-600 mt-2 bg-gray-200 p-3 rounded-3xl text-xs text-left">
            <span className='lesson w-[26%] bg-[url(/cardIcons/note-2.svg)] bg-no-repeat bg-right'>{"۱۲ درس"}</span>
            <span className='course-time w-[28%] bg-[url(/cardIcons/clock.svg)] bg-no-repeat bg-right'>{"۵ ساعت"}</span>
            <span className='course-date w-[33%] bg-[url(/cardIcons/calendar-2.svg)] bg-no-repeat bg-right'>{"ا آذر 1402"}</span>
          </div>
  
          <div className="flex justify-between mt-3 text-sm gap-3" >
            <span className="teacher text-gray-500">{"مدرس: دکتر بحرالعلوم"}</span>
