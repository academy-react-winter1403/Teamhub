...
const ReplyComment = ({ commentId }: replyProps) => {
  ...
  return (
    <div className="relative">
      ...
      />
      <button
        className="btn h-8 bg-blue-500 text-white text-xs rounded-xl absolute -top-10 left-10"
      ...
      {replys.map((item, index) => (
        <div
          key={index}
          className="border-r-1 border-b-1 border-gray-300 rounded-xl w-[85%] mr-30 p-2 mb-2"
        >
          ...
          <p className="text-right text-sm text-gray-700 mb-3">
            {item.describe}
          </p>
